...
**当前发布基线：v0.8.0 已发布（网页 + macOS 桌面）**。公开仓库为 https://github.com/oukeming64-tech/direct-light，GitHub Pages 自动部署和 Tauri macOS Release CI 均已落地；`src-tauri/Cargo.lock` 已提交，桌面构建使用锁文件保持可复现。**当前工作树：v0.9 自定义灯具预设导入/导出已完成并通过用户真机验收（2026-06-23）**，TopBar 显示 `v0.9.0`；`package.json` / `tauri.conf.json` / `Cargo.toml` 仍保持 `0.8.0`，等待下一次正式网页 / 桌面发版时再同步。个人复盘 `个人重构/` 继续保留本地、不入仓库。后续主线：多语言 UI，等核心功能和字段更稳定后再抽取文案。
...
**v0.9d 收口已完成，v0.9 通过用户视觉验收（Claude Code，Codex 复核，2026-06-23）**：v0.9c 全部 8 步用户肉眼验收通过（下拉分组、存当前灯为器械、切换种参、导出/导入、删除回退、刷新持久化、坏 json 不崩）。收口：① **携带验证**——保存方案 `structuredClone(scene)` + `normalizeLight` 的 `...light` 保留 `fixturePresetId`；复制灯 `...structuredClone(src)` 保留；A/B 用 scene 克隆携带、`sceneDiff` 无 fixture 引用（按 spec §8，器械不同时 type/intensity 等已让「灯光」chip 高亮，无需新增 diff 类别）；旧方案引用已删自定义器械时 `findFixtureById`→undefined 自动回退「自定义参数」、不崩，`sceneMigration` 无需改；Codex 复核补一处 UI 兜底，让器械下拉在 resolved fixture 缺失时也显式回到「自定义参数」。② TopBar 版本 `v0.8.0 → v0.9.0`（产品迭代号；`package.json`/`tauri.conf.json`/`Cargo.toml` 仍 `0.8.0`——按历史惯例 TopBar 跟产品迭代、四件套版本号跟实际发布；桌面/网页正式发版是独立一步，本轮**未** push/tag/release，避免再踩 v0.8 的 `Cargo.lock` 漂移坑）。③ 全套文档同步到「v0.9 完成」（COLLABORATION/V0_9 spec/ROADMAP/CLAUDE/AGENTS/ARCHITECTURE/CHANGELOG/README*/CONTRIBUTING/HERMES）。`tsc·lint·build` 全绿。**v0.9 自定义灯具导入/导出全线完成；后续主线：多语言 UI。** 分工说明：本刀主要是验证 + 版本号 + 文档；用户已重申「能甩 OpenRouter 就甩」，后续凡有可规格化的代码草稿默认交 OpenRouter 起草、Claude 审核集成。
...
23. **[后续] 多语言 UI**：下一条主线。在核心功能和字段更稳定后再抽取简体中文 / 英语 / 日语文案；保存方案、A/B 和场景数据不能依赖语言。
...
1. 多语言 UI：抽取简体中文 / 英语 / 日语 UI 文案，并验证切换语言不改变场景、A/B 或保存方案。
2. 发布收口（如需要）：把工作树 v0.9 推送后，再另行决定是否切正式网页 / 桌面 release，并同步 `package.json` / Tauri / Cargo 版本号。
