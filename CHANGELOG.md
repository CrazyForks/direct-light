...
- **野獣博士先輩の協力のもと、日本語の読みやすさを改善しました。**
- オンボーディング、ライト、人物、カメラ、白ホリ、プリセット、A/B比較、組み込み表示名の直訳調を見直し、日本の撮影・制作ツールとして自然に読める表現へ整えました。
- 変更は既存の日本語メッセージに限定しています。翻訳キー、typed i18n の構成、シーン / プリセット / A/B / カスタム器具のデータ、レンダリング、英語・中国語の文言は変更していません。

## [1.0.4] - 2026-07-10 · 新手导览 + 体验与可靠性收口

> 通过 Codex 确定性检查与真实浏览器验收；TopBar / `package.json` / `package-lock.json` / Tauri / Cargo / Cargo.lock 均为 `1.0.4`，tag `v1.0.4`。

- 主应用新增五步首次导览：简中 / 英文 / 日文完整文案，支持跳过、完成记忆、Escape 关闭、焦点归还与顶栏 `?` 随时重看；灯光「强度 / 位置」前移到右栏首屏，快速布光示例在常见笔记本宽度改为紧凑选择器。
- 体验与可访问性收口：960px 以下不再把预演画布挤成 0px，而是明确提示横屏或扩大窗口；对象行、已存方案、滑杆、分段选择、开关和页面语言获得正确的键盘 / 读屏语义。
- 修复 A/B 摘要漏判：光束角、距离、目标、器械、人物外观、道具尺寸 / 材质 / 显隐、摄影机目标和白棚渲染设置等变化现在都会进入正确分类；被动 B 画面改为按需渲染，阴影贴图只在分辨率变化时重建，减少对比视图和连续拖动滑杆的 GPU 压力。
- 修复浏览器本地存储“假成功”：方案保存 / 复制 / 删除、自定义器械保存 / 导入 / 删除现在只在 localStorage 写入成功后更新内存；空间不足或存储被禁用时保留原状态，并用三语状态提示明确告知失败。
- 修复自定义 tube 器械在 3D 中被画成 panel；器械外形现在与 UI 共用内置 + 自定义统一 lookup。GLB 人物改用 `SkeletonUtils.clone`，避免带骨骼模型的多人实例共享错误骨架。
- Vite 8 / Rolldown 生产构建按 React / Three / R3F-Drei 边界拆包并限制分块尺寸；主应用脚本从约 1.19 MB 单块拆成多个不超过约 373 kB 的块，构建不再出现 500 kB 警告。
- 重做 `/showcase/` 展示页：保留 Apple 式的克制、尺度与滚动节奏，但改成 Direct Light 自己的黑色工作台视觉；真实产品视频进入首屏，灯光 / 阴影 / 镜头截图可交互切换，并新增能力卡片、三步工作流、经过重新编排的中英文文字层级、移动端布局、键盘焦点和减少动效支持。
...
- 建立文档同步规则：每次仓库修改都必须更新直接相关文档，并明确记录「改了什么」和「没改什么」。
- 整理项目文档：旧阶段规格归档到 `docs/history/specs/`，长版根文档快照归档到 `docs/history/snapshots/`，根目录 README / ROADMAP / ARCHITECTURE / RENDERING_SPEC 改成短版入口。
