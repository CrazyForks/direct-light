...
状态：**已落地并通过用户视觉验收（2026-06-21）**。Claude Code 完成 v0.6e 收口实现与回归，Codex 负责进度文档（`COLLABORATION.md` / `CLAUDE.md` / `ROADMAP.md` / `HERMES.md`）和可见版本号同步；v0.6 控光线全线完成，下一步进入 v0.7。创建日期：2026-06-21；定稿日期：2026-06-21；验收日期：2026-06-21。父规格：`V0_6_MODIFIER_SPEC.md §2`（v0.6 拆分，v0.6e = 收口验收）、`V0_6D_OPTICS_SPEC.md §12`（完成后进入哪里）。

## 1. 目标

v0.6a–d 已经把「灯上附件」和「棚内控光器材 + 近似光学」做完并通过用户视觉验收。v0.6e 不加新光学、不加新器材，只做**收口**：让整条 v0.6 控光线在 A/B 对比、保存/加载/复制、导出截图、文档四个面上自洽稳定，达到「可以进入 v0.7 开源第一版」的状态。
...
- 多语言（开源后 v0.8）。
- v0.7 的开源第一版动作本身（README/LICENSE 正式落地、可选桌面封包）——v0.6e 只把「开源前限制说明」写清，正式开源在 v0.7。
...
1. `COLLABORATION.md` / `CLAUDE.md` / `ROADMAP.md` / `HERMES.md`：把 v0.6a–e 标为完成并通过验收，下一步指向 v0.7；可见版本号（`TopBar`）在 v0.6e 收口通过后升到 `v0.6e`（或按 Codex 判断直接进入 v0.7 标识）。
...
v0.6e 收口通过后，v0.6 控光线全线完成 → 进入 **v0.7：可开源第一版 + 可选桌面封包**。开源第一版不等待多语言（v0.8）或更多光源（v0.9）。
